...
class StorePractitionerApplicationRequest extends FormRequest
{
    ...
    /**
     * Strip all HTML from every text field before validation.
     *
     * None of these fields are rich-text editors — they are plain text inputs
     * that get rendered in the admin panel and in notification emails.
     * A single injected <script> or <img onerror> in bio/professional_title
     * would execute in the admin's session when they review the application.
     */
    protected function prepareForValidation(): void
    {
        ...
        foreach ($plainTextFields as $field) {
            if ($this->has($field)) {
                $merge[$field] = $sanitizer->sanitizePlainText($this->input($field));
            }
        }

        // Sanitize credential text fields
        if ($this->has('credentials') && is_array($this->input('credentials'))) {
            ...
            foreach ($credentials as $i => $credential) {
                if (isset($credential['license_number'])) {
                    $credentials[$i]['license_number'] = $sanitizer->sanitizePlainText($credential['license_number']);
                }
                if (isset($credential['issuing_organization'])) {
                    $credentials[$i]['issuing_organization'] = $sanitizer->sanitizePlainText($credential['issuing_organization']);
                }
            }
        ...
        if ($this->has('availability_schedule') && is_string($this->availability_schedule)) {
            $merge['availability_schedule'] = json_decode($this->availability_schedule, true) ?? [];
        }

        // Decode selected_services JSON string → array
        if ($this->has('selected_services') && is_string($this->selected_services)) {
            $merge['selected_services'] = json_decode($this->selected_services, true) ?? [];
        }
    ...
    public function rules(): array
    {
        return [
            ...
            // Credentials — explicit MIME type check via finfo, not just extension
            ...
            'credentials.*.file'                   => [
                ...
                'min:1',
                'max:5120',
                // Validate by actual MIME type (finfo), not just extension
                'mimes:pdf,jpg,jpeg,png',
                function ($attribute, $value, $fail) {
                    if ($value->getSize() === 0) {
                        $fail('The uploaded file cannot be empty.');
                    }
                    // Double-check extension as secondary guard
                    $allowed = ['pdf', 'jpg', 'jpeg', 'png'];
                    if (! in_array(strtolower($value->getClientOriginalExtension()), $allowed)) {
                        $fail('Credentials must be PDF or image files (JPG, PNG).');
                    }
                    // Block PHP/executable extensions disguised as images
                    $dangerous = ['php', 'php3', 'php4', 'php5', 'phtml', 'exe', 'sh', 'bat', 'js'];
                    if (in_array(strtolower($value->getClientOriginalExtension()), $dangerous)) {
